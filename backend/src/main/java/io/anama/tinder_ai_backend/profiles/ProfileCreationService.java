...
public class ProfileCreationService {
    ...
    private Profile generateProfileImage(Profile profile) {
        ...
        String prompt = STR
                ."Selfie of a \{profile.age()} year old \{profile.myersBriggsPersonalityType()} \{profile.ethnicity()} \{profile.gender()}, \{randomSelfieType}, photorealistic skin texture and details, individual hairs and pores visible, highly detailed, photorealistic, hyperrealistic, subsurface scattering, 4k DSLR, ultrarealistic, best quality, masterpiece. Bio- \{profile.bio()}";
        ...
        """;
        System.out
                .println(STR."Creating image for \{profile.firstName()} \{profile.lastName()}(\{profile.ethnicity()})");

        // Make a POST request to the Stable diffusion URL
                        HttpRequest httpRequest = this.stableDiffusionRequestBuilder.POST(
                                HttpRequest.BodyPublishers.ofString(jsonString)).build();
    ...
    public Function<Profile, Boolean> saveProfile() {
        return (Profile profile) -> {
            System.out.println(
                    "This is the function that we expect to be called by Spring AI by looking at the OpenAI response");
